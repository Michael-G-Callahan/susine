...
#' Compute univariate association z-scores
#'
#' @description Computes t-statistics (betahat / sebetahat) for association
#'   between Y and each column of X using `univariate_regression`.
#'
#' @param X n x p matrix of regressors.
#' @param Y Response vector (length n) or matrix (n x m). If matrix, returns
#'   a p x m matrix of z-scores.
#' @param center Logical; center X and Y.
#' @param scale Logical; scale X and Y.
#'
#' @return Length-p vector (or p x m matrix) of z-scores.
#' @keywords internal
